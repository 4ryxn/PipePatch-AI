...
REPAIR_METHOD_ID: Final[Literal["two_slip_coupling_section_replacement"]] = (
    "two_slip_coupling_section_replacement"
)
SAFETY_WARNINGS = [
...
def assess_repair(
    ...
    if analysis.is_mock:
        needs_information.append(
            "A live AI analysis is required; demo analysis results cannot qualify."
        )
    ...
    _gate(
        confirmations.outdoor_irrigation, "outdoor irrigation line", professional, needs_information
    )
    _gate(
        confirmations.water_supply_shut_off,
        "water supply shut off and line made safe",
        professional,
        needs_information,
    )
    _gate(
        confirmations.pvc_schedule_40_marking,
        "visible PVC Schedule 40 marking",
        professional,
        needs_information,
    )
    if confirmations.nominal_size is None:
        needs_information.append(
            "Select a visibly confirmed nominal size: 1/2 in, 3/4 in, or 1 in."
        )
    _gate(
        confirmations.clean_transverse_cut,
        "one clean transverse cut",
        professional,
        needs_information,
    )
    _gate(
        confirmations.no_additional_damage,
        "absence of cracks, crushing, deformation, or missing fragments",
        professional,
        needs_information,
    )
    _gate(confirmations.straight_section, "straight pipe section", professional, needs_information)
    _gate(
        confirmations.safely_away_from_components,
        "distance from valves, fittings, manifolds, foundation penetrations, and other utilities",
        professional,
        needs_information,
    )
    _gate(
        confirmations.pipe_ends_accessible,
        "both pipe ends safely exposed and accessible",
        professional,
        needs_information,
    )
    if professional:
    ...
    return RepairAssessmentResponse(
        decision=RepairDecision.ELIGIBLE,
        reasons=[
            "All deterministic safety gates are explicitly confirmed for this limited MVP case."
        ],
        safety_warnings=SAFETY_WARNINGS,
        confirmed_pipe_size=confirmations.nominal_size,
        repair_method_id=REPAIR_METHOD_ID,
        parts=[
            "Same-size Schedule 40 PVC replacement pipe.",
            "Two same-size Schedule 40 PVC repair/slip couplings without internal stops.",
            "PVC primer where required.",
            "PVC solvent cement compatible with the pipe and fittings.",
        ],
        tools=["Measuring tool.", "PVC cutter.", "Deburring tool.", "Gloves.", "Eye protection."],
    )


def _gate(
    answer: Confirmation, label: str, professional: list[str], needs_information: list[str]
) -> None:
...
def _response(
    decision: RepairDecision, reasons: list[str], confirmations: RepairConfirmations
) -> RepairAssessmentResponse:
    return RepairAssessmentResponse(
        decision=decision,
        reasons=reasons,
