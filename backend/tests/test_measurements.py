...
def image() -> ValidatedImage:
    ...
    marker = cv2.aruco.generateImageMarker(
        cv2.aruco.getPredefinedDictionary(MARKER_DICTIONARY), EXPECTED_MARKER_ID, 250
    )
...
def measured(diameter: float, gap: float):
    return measure_image(
        image(),
        ImagePoint(x=320, y=400),
        ImagePoint(x=320 + diameter, y=400),
        ImagePoint(x=320, y=500),
        ImagePoint(x=320 + gap, y=500),
    )
...
@pytest.mark.parametrize(
    ("pixels", "size"),
    [
        (106.5, NominalPipeSize.HALF),
        (133.5, NominalPipeSize.THREE_QUARTER),
        (167, NominalPipeSize.ONE),
    ],
)
...
def test_short_out_of_bounds_and_ambiguous_lines_are_safe() -> None:
    ...
    out = measure_image(
        image(),
        ImagePoint(x=-1, y=1),
        ImagePoint(x=100, y=1),
        ImagePoint(x=1, y=1),
        ImagePoint(x=200, y=1),
    )
...
@pytest.mark.parametrize(
    ("gap", "status"),
    [
        (100, GapRangeStatus.BELOW_MVP_RANGE),
        (250, GapRangeStatus.WITHIN_MVP_RANGE),
        (500, GapRangeStatus.ABOVE_MVP_RANGE),
    ],
)
...
def test_measurement_endpoint_validates_required_coordinate_fields() -> None:
    response = client.post(
        "/api/v1/measurements", files={"image": ("ignored.png", image().content, "image/png")}
    )
