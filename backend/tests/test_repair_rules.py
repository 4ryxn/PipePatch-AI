...
def confirmations(**changes: object) -> RepairConfirmations:
    data: dict[str, object] = {
        "line_type": LineType.OUTDOOR_IRRIGATION,
...
@pytest.mark.parametrize(
    "confidence, decision",
    [
        (0.749, RepairDecision.NEEDS_MORE_INFORMATION),
        (0.75, RepairDecision.ELIGIBLE),
        (0.751, RepairDecision.ELIGIBLE),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        "outdoor_irrigation",
        "water_supply_shut_off",
        "pvc_schedule_40_marking",
        "clean_transverse_cut",
        "no_additional_damage",
        "straight_section",
        "safely_away_from_components",
        "pipe_ends_accessible",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        LineType.GAS,
        LineType.SEWER,
        LineType.ELECTRICAL_CONDUIT,
        LineType.POTABLE_HOUSEHOLD,
        LineType.UNKNOWN,
    ],
)
def test_prohibited_or_unknown_line_types_require_professional(line_type: LineType) -> None:
    assert (
        assess_repair(live_analysis(), confirmations(line_type=line_type)).decision
...
def test_missing_size_mock_and_unsupported_analysis_need_information() -> None:
    assert (
        assess_repair(live_analysis(), confirmations(nominal_size=None)).decision
        is RepairDecision.NEEDS_MORE_INFORMATION
    )
    assert (
        assess_repair(live_analysis(is_mock=True), confirmations()).decision
        is RepairDecision.NEEDS_MORE_INFORMATION
    )
    assert (
        assess_repair(live_analysis(supported_case=False), confirmations()).decision
        is RepairDecision.NEEDS_MORE_INFORMATION
    )
