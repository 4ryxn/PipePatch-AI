...
Accounts are optional. The complete repair workflow remains available without signing in. If enabled, authentication uses server-side Argon2 password hashes and short-lived signed JWT access tokens. The web client keeps an active token only in browser session memory and clears it on sign-out or authentication expiry; it does not use photo storage.
...
For local development set `AUTH_ENABLED=true`, `DATABASE_URL=sqlite:///./pipepatch.db`, and generate a secret, for example: `python -c "import secrets; print(secrets.token_urlsafe(48))"`. Deployment may use PostgreSQL through `DATABASE_URL`; use a strong private secret and TLS. Never place a secret in browser configuration, source control, or chat.
