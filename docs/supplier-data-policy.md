...
When enabled, the backend makes one Nominatim geocode request and one Overpass POI query, never from the mobile app. It uses a descriptive configured User-Agent, a short process-local TTL cache, and process-wide Nominatim pacing of at most one request each second. Results use public OpenStreetMap data and show `© OpenStreetMap contributors` attribution. They are incomplete, approximate points, not a current or comprehensive supplier directory.
