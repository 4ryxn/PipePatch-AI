...
import { saveHistory } from "./src/services/authService";
...
import type { HistorySummary } from "./src/types/auth";
...
export default function App(): React.JSX.Element {
  ...
  const saveRepairSummary = (): void => { if (!guidance || !measurement || !isComplete(confirmations)) return; const summary: HistorySummary = { outcome: guidance.decision === "eligible" ? "supported" : "rejected", confirmed_nominal_size: confirmations.nominal_size, repair_method_id: guidance.repair_method_id, measured_gap_range_status: measurement.gap_range_status, generic_parts_item_names: guidance.materials_tools_checklist, safety_and_limitation_text: [...guidance.stop_conditions, ...guidance.limitations] }; void saveHistory("Pipe repair assessment", summary).catch(() => setError("Sign in to save this text-only repair summary.")); };

  if (accountOpen) return <SafeAreaProvider><Screen><AccountScreen onBack={() => setAccountOpen(false)} /></Screen></SafeAreaProvider>;
  ...
  if (guidance) return <SafeAreaProvider><Screen><RepairGuidanceScreen guidance={guidance} onBack={() => setGuidance(null)} onParts={() => startParts()} onSave={saveRepairSummary} /></Screen></SafeAreaProvider>;
  return <SafeAreaProvider><Screen><StatusBar style="dark" />
    {flow.screen === "home" && <Home healthState={healthState} onStart={() => dispatch({ type: "START" })} onAccount={() => setAccountOpen(true)} />}
...
function Home({ healthState, onStart, onAccount }: { healthState: HealthState; onStart: () => void; onAccount: () => void }): React.JSX.Element { return <><Text style={s.eyebrow}>LOCAL PHOTO PREP</Text><Text style={s.title}>PipePatch AI</Text><Text style={s.body}>Prepare one clear pipe-damage photo for the configured backend analysis. Results never include repair advice.</Text><View style={s.card}><Text style={s.cardTitle}>Backend health</Text>{healthState === "checking" ? <ActivityIndicator accessibilityLabel="Checking backend health" color={colors.green} /> : <Text accessibilityLiveRegion="polite" style={healthState === "healthy" ? s.ok : s.warning}>{healthState === "healthy" ? "Connected" : "Unavailable — photo preparation still works locally"}</Text>}</View><View style={s.spacer} /><AppButton label="Prepare a pipe photo" onPress={onStart} /><AppButton label="Account & saved summaries" variant="secondary" onPress={onAccount} /></>; }
