...
type Props = { analysis: AnalysisResponse; onStartOver: () => void; onContinue?: () => void; onCalibration?: () => void };

export function AnalysisResultScreen({ analysis, onStartOver, onContinue, onCalibration }: Props): React.JSX.Element {
  ...
  const category = analysis.damage_category ?? "unknown_or_unsupported";
  const cleanCut = category === "clean_transverse_cut";
  return <><Text style={styles.eyebrow}>{label}</Text><Text style={styles.title}>Upload flow complete</Text><View style={styles.warning}><Text style={styles.warningTitle}>{warning}</Text><Text style={styles.body}>{analysis.summary}</Text></View><View style={styles.card}><Detail label="Observed damage category" value={category.replaceAll("_", " ")} /><Detail label="Confidence" value={`${Math.round(analysis.confidence * 100)}%`} /><Detail label="Material" value={analysis.material ?? "Unknown"} /><Detail label="Schedule" value={analysis.pipe_schedule ?? "Unknown"} /></View>{!cleanCut && <View style={styles.warning}><Text style={styles.warningTitle}>DIY repair guidance is not available for this damage type yet</Text><Text style={styles.body}>Retake the photo if the observed category is unclear, or seek professional help for active leaks, fittings, valves, or uncertain damage.</Text></View>}<List title="Evidence" items={analysis.evidence} /><List title="Unknowns" items={analysis.unknowns} /><List title="Safety flags" items={analysis.safety_flags} /><View style={styles.card}><Text style={styles.cardTitle}>Next action</Text><Text style={styles.body}>{analysis.next_action}</Text></View><View style={styles.spacer} />{onCalibration && <AppButton label="Set reference scale" variant="secondary" onPress={onCalibration} />}{onContinue && cleanCut && <AppButton label="Confirm safety details" onPress={onContinue} />}<AppButton label="Prepare another photo" variant={onContinue && cleanCut ? "secondary" : "primary"} onPress={onStartOver} /></>;
}
