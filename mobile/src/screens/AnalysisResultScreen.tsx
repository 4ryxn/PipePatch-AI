...
type Props = { analysis: AnalysisResponse; onStartOver: () => void };

export function AnalysisResultScreen({ analysis, onStartOver }: Props): React.JSX.Element {
  return <><Text style={styles.eyebrow}>DEMO ANALYSIS</Text><Text style={styles.title}>Upload flow complete</Text><View style={styles.warning}><Text style={styles.warningTitle}>Demo analysis — not real AI</Text><Text style={styles.body}>{analysis.summary}</Text></View><View style={styles.card}><Detail label="Supported case" value={analysis.supported_case ? "Yes" : "No — demo only"} /><Detail label="Material" value={analysis.material ?? "Unknown"} /><Detail label="Schedule" value={analysis.pipe_schedule ?? "Unknown"} /><Detail label="Nominal size" value={analysis.nominal_size ?? "Not measured"} /><Detail label="Damage type" value={analysis.damage_type ?? "Unknown"} /><Detail label="Confidence" value={`${Math.round(analysis.confidence * 100)}%`} /></View><List title="Evidence" items={analysis.evidence} /><List title="Unknowns" items={analysis.unknowns} /><List title="Safety flags" items={analysis.safety_flags} /><View style={styles.card}><Text style={styles.cardTitle}>Next action</Text><Text style={styles.body}>{analysis.next_action}</Text></View><View style={styles.spacer} /><AppButton label="Prepare another photo" onPress={onStartOver} /></>;
}
