...
type Props = { value: Draft; suggestedSize?: "1/2" | "3/4" | "1" | null; onChange: (value: Draft) => void; onSubmit: () => void };
...
export function RepairConfirmationScreen({ value, suggestedSize, onChange, onSubmit }: Props): React.JSX.Element {
  ...
  return <>
    ...
    {suggestedSize && <Text style={styles.body}>Measurement context only: closest visible-diameter suggestion is {suggestedSize} in. You must still select a size you can visibly confirm.</Text>}<Question label="Visibly confirmed nominal size" value={selectedSize} options={[["1/2", "1/2 in"], ["3/4", "3/4 in"], ["1", "1 in"], ["unknown", "Unknown"]]} onSelect={(size) => onChange({ ...value, nominal_size: size === "unknown" ? null : size as NominalPipeSize })} />
