...
import { isRequestCancellation, requestAnalysis } from "./analysisService";
...
test("sends the confirmed image as multipart without manually setting a Content-Type boundary", async () => {
  await expect(requestAnalysis(image, new AbortController().signal)).resolves.toEqual(response);
...
test("accepts a typed live analysis response as well as mock data", async () => {
  const liveResponse = { ...response, is_mock: false };
  mockFetch.mockResolvedValue({ ok: true, json: async () => liveResponse });

  await expect(requestAnalysis(image, new AbortController().signal)).resolves.toEqual(liveResponse);
});
