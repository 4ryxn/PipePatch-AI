...
export async function cleanupAppOwnedUris(uris: string[], remove: (uri: string) => Promise<void> = (uri) => FileSystem.deleteAsync(uri, { idempotent: true })): Promise<void> { await Promise.all(uris.map(async (uri) => { try { await remove(uri); } catch { /* best effort */ } })); }
