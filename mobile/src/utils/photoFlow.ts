...
export function photoFlowReducer(state: PhotoFlowState, event: PhotoFlowEvent): PhotoFlowState {
  switch (event.type) {
    ...
    case "REPLACE": return state.screen === "review" || state.screen === "analysis_error" ? { ...state, screen: "source", image: null, ready: null, analysis: null, assessment: null, operationId: event.operationId } : state;
